...
### 4. `test_weights.csv` - משקלים למבחנים

קובץ זה מגדיר את המשקל (באחוזים) לכל מבחן עבור מגדר מסוים. רק מבחנים עם משקל גדול מ-0 יוצגו בעמוד הציון הסופי.

**העמודות:**
- `test_type` - מזהה המבחן (מתאים ל-value ב-fields.csv)
- `label` - התווית בעברית
- `weight_percent` - המשקל באחוזים
- `gender` - המגדר (boys/girls, case-insensitive)

```csv
test_type,label,weight_percent,gender
2000m,ריצת 2000 מטר,60,boys
pullups,מתח,20,boys
seat_ups,כפיפות בטן,20,boys
...
2000m,ריצת 2000 מטר,60,girls
pushups,שכיבות סמיכה,20,girls
seat_ups,כפיפות בטן,20,girls
...
```
...
- הערה: עבור בדיקת ביפ, הרמות מיוצגות בפורמט x.y או x.yy, לדוגמה 6.01 עבור רמה 6 משוט 1, ו-6.10 עבור רמה 6 משוט 10.
...
הציון הסופי מחושב כממוצע משוקלל של הציונים הפרטניים, כאשר המשקלים הם:
- **לבנים:** ריצת 2000 מטר (60%), מתח (20%), כפיפות בטן (20%)
- **לבנות:** ריצת 2000 מטר (60%), שכיבות סמיכה (20%), כפיפות בטן (20%)

**נוסחת החישוב:**
```
ציון_סופי = floor( (ציון_מבחן1 × משקל_מבחן1 + ציון_מבחן2 × משקל_מבחן2 + ...) / 100 )
```

לדוגמה, לבן עם ציון 85 בריצת 2000 מטר, 90 במתח, ו-88 בכפיפות בטן:
```
ציון_סופי = floor( (85 × 60 + 90 × 20 + 88 × 20) / 100 ) = floor( (5100 + 1800 + 1760) / 100 ) = floor(8660 / 100) = floor(86.6) = 86
```

הערות: ה־CSV `test_weights.csv` מכיל שדות `test_type`, `label` ו־`weight_percent` בדיוק כמו בדף המחשבון, אבל בחישוב ציון סופי משוקלל נשקלים רק הפריטים עם משקל חיובי.
...
**טיפים:**
- הקובץ חייב להכיל 61 שורות (ציונים 100-40)
...
- קובץ התצורה `test_weights.csv` מגדיר את המשקל (אחוז) לכל `test_type` עבור מגדר מסוים.
- המשקלים מנורמלים אוטומטית כך שיסכמו ל-100% לפני החישוב.
- רק מבחנים עם `weight_percent` גדול מ-0 יוצגו בעמוד ה-`finalGrade` ויתבקשו תוצאותיהם.
- אם קובץ ה-`scores/<test_type>.csv` חסר עבור מבחן נתון, אותו מבחן מדלגים ממנו והמשקל מנורמל מחדש.
...
- לכל מבחן המערכת ממפה את הביצוע לציון בין 100 ל-40 באמצעות קובץ ה-`scores` המתאים.
- הציון הסופי המחושב הוא ממוצע משוקלל של ציוני המבחנים (כאשר כל ציוני מבחן מוכפלים במשקלם המנורמל), ולאחר מכן מעוגל כלפי מטה.
- אם מבחן חסר קובץ ציונים, הוא מדלג והמשקלים מנורמלים מחדש.
