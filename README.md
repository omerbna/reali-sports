...
הדף מיועד לחישוב ציון סופי עבור **כיתה יב' בלבד**. הוא **לא כולל שדה לבחירת שכבה**; השכבה מוגדרת בקוד כ־`12`. משתמשים עדיין מתבקשים לבחור **מגדר** (בנים/בנות) לפני החישוב. רק מבחנים שיופיעו ב־`test_weights.csv` עם משקל גדול מ‑0 יוצגו ויישמשו בחישוב הציון הסופי.
